...
pub const IntList = struct {
    ...
    fn add(self: *IntList, value: i64) !void {
        ...
        if (pos == len) {
            ...
            // AHA! This is where the leak occurs; we allocate for the new
            // larger array but never free the previous array. Fix it with
            self.allocator.free(self.items);
...
// as written, this implements dynamic arrays (basically a python list) but
// with a memory leak!!
// The following test will show us what's up
